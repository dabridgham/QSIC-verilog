...
the original RP11-C hardware actually responds from $0776700_8$ up to
$0776736_8$, but there are no actual registers from $0776700_8$ to
$0776706_8$, so we can put any additional registers needed in that range.
...
is 5. Both the interrupt priority and vector are configurable (not yet).
...
\begin{bittable}
  ...
  data during a Read Check or Read function. The RP11-F does not do
  ...
  to initiate a transfer to a cylinder larger than $624_8$. \\
...
\begin{bittable}
  00 & Go & When set, causes the RP11-D to act on the function
    ...
    \footnote{The RP11-C manual lists GO as write-only. The RP11-C
    allows it to be read. Oh wait, I know why they did that.
...
\begin{bittable}
  ...
  for data transfers.  The MEX bits (bits 04 and 05 of RPCS) extend
  the address to 18-bits and, if enabled, the BAE bits (bits 00-05 of
  RPXA) extend the address to 22-bits (QSIC only). Bit 00 is always 0
